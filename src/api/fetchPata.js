/**
 * API module – handles fetching podcast data and normalisation.
 * Includes a fallback dataset when the API is down.
 */
import { genres } from "../data.js";

const API_BASE = "https://podcast-api.netlify.app";

/** Map genre IDs to titles. Used for the preview list only. */
export const GENRE_MAP = Object.fromEntries(genres.map((g) => [g.id, g.title]));

/** Normalise a raw podcast preview. */
...
/** Fallback dataset (10 shows) – used when the API fails. */
const FALLBACK_DATA = [
  {
    id: "10716",
    title: "Something Was Wrong",
    description: "True-crime docuseries.",
    seasons: 14,
    image:
  ...
  },
  {
    ...
    description: "Extraordinary true stories.",
    seasons: 12,
    image:
  ...
  },
  {
    id: "9177",
    title: "Killer Psyche",
    description: "FBI agent explores criminal minds.",
    seasons: 2,
    image:
  ...
  },
  {
    ...
    description: "British elite scandals.",
    seasons: 19,
    image:
...
/** Fetches previews from the API; falls back to local data on error. */
export async function fetchPodcasts(setPodcasts, setError, setLoading) {
  try {
    const res = await fetch(API_BASE);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    const normalized = Array.isArray(data) ? data.map(normalizePodcast) : [];
...
/** Fetches a single show by ID (full details + seasons). */
